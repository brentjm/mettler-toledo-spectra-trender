...
class OPCUACertificateManager extends node_opcua_pki_1.CertificateManager {
    ...
    checkCertificate(certificate, callback) {
        const checkCertificateStep2 = (err) => {
            if (err) {
                return callback(err, node_opcua_constants_1.StatusCodes.BadInternalError);
            }
            super.verifyCertificate(certificate, (err1, status) => {
                if (err1) {
                    return callback(err1);
                }
                const statusCode = node_opcua_constants_1.StatusCodes[status];
                if (!statusCode) {
    ...
    getTrustStatus(certificate, callback) {
        this.isCertificateTrusted(certificate, (err, trustedStatus) => {
            callback(err, err ? undefined : node_opcua_constants_1.StatusCodes[trustedStatus]);
        });
    }
}
...
function checkCertificateValidity(certificate) {
    ...
    if (!certificate) {
        // missing certificate
        return node_opcua_constants_1.StatusCodes.BadSecurityChecksFailed;
    }
    ...
    if (cert.notBefore.getTime() > now.getTime()) {
        ...
        return node_opcua_constants_1.StatusCodes.BadCertificateTimeInvalid;
    }
    ...
    return node_opcua_constants_1.StatusCodes.Good;
}
