...
function composeResult(nodes, nodesToRead, dataValues) {
    ...
    for (const node of nodes) {
        const data = {
            nodeId: node_opcua_nodeid_1.resolveNodeId(node),
            statusCode: node_opcua_constants_1.StatusCodes.BadNodeIdUnknown
        };
        let addedProperty = 0;
        for (const key of attributeNames) {
            ...
            c++;
            if (dataValue.statusCode.equals(node_opcua_constants_1.StatusCodes.Good)) {
        ...
        /* istanbul ignore if */
        if (addedProperty > 0) {
            data.statusCode = node_opcua_constants_1.StatusCodes.Good;
        }
        else {
            data.statusCode = node_opcua_constants_1.StatusCodes.BadNodeIdUnknown;
        }
        results.push(data);
    }
    return results;
...
class ClientSessionImpl extends events_1.EventEmitter {
    ...
    readVariableValue(...args) {
        ...
        this.performMessageTransaction(request, (err, response) => {
            ...
            /* istanbul ignore next */
            if (response.responseHeader.serviceResult.isNot(node_opcua_constants_1.StatusCodes.Good)) {
    ...
    getMonitoredItems(...args) {
        ...
        this.call(methodsToCall, (err, result) => {
            ...
            /* istanbul ignore next */
            if (result.statusCode.isNot(node_opcua_constants_1.StatusCodes.Good)) {
    ...
    getBuiltInDataType(...args) {
        ...
        this.read(nodeToRead, 0, (err, dataValue) => {
            ...
            /* istanbul ignore next */
            if (dataValue.statusCode.isNot(node_opcua_constants_1.StatusCodes.Good)) {
    ...
    readNamespaceArray(...args) {
        ...
        this.read({
            ...
            /* istanbul ignore next */
            if (dataValue.statusCode !== node_opcua_constants_1.StatusCodes.Good) {
    ...
    addCommentCondition(conditionId, eventId, comment, callback) {
    }
    confirmCondition(conditionId, eventId, comment, callback) {
    }
    acknowledgeCondition(conditionId, eventId, comment, callback) {
    }
    findMethodId(nodeId, methodName, callback) {
    }
    _callMethodCondition(methodName, conditionId, eventId, comment, callback) {
    }
    _defaultRequest(requestClass, responseClass, options, callback) {
        node_opcua_assert_1.assert(_.isFunction(callback));
        ...
        this.performMessageTransaction(request, (err, response) => {
            ...
            if (err) {
                ...
                if (err.message.match(/BadSessionClosed/)) {
                     ...
                     */
                    this.emitCloseEvent(node_opcua_constants_1.StatusCodes.BadSessionClosed);
                }
                return callback(err, response);
            }
            callback(null, response);
...
function promoteOpaqueStructure(session, dataValues) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const promises = dataValuesToFix.map((dataValue) => __awaiter(this, void 0, void 0, function* () {
            yield node_opcua_client_dynamic_extension_object_1.resolveDynamicExtensionObject(dataValue.value, sessionPriv.$$extraDataTypeManager);
        }));
        yield Promise.all(promises);
    });
}
const promoteOpaqueStructureWithCallback = util_1.callbackify(promoteOpaqueStructure);
